...
RSpec.describe Shipment, type: :model do
  ...
  describe '#group_item_descriptions' do
    ...
    shared_examples 'orders correctly' do
    ...
    context 'when shipments items have not loaded yet' do
      let(:result) { shipment.group_item_descriptions(order).as_json(except: :id) }

      it_behaves_like 'orders correctly'
    end

    context 'when shipments already loaded' do
      let(:result) do
        described_class.includes(:shipment_items)
                       .find(shipment.id)
                       .group_item_descriptions(order)
                       .map { |item| item.to_h.with_indifferent_access }
      end
